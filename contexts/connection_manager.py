...
class FrontdoorChatter(ContextDecorator):
    ...
    def speak(self, message: dict):
        self.sock.send(
            FrontdoorChatter.add_header(json.dumps(message, separators=(",", ":")))
        )

    def ping(self, timestamp=555):
        self.sock.send(
            bytes([3, 2]) + (4).to_bytes(4, "little") + timestamp.to_bytes(4, "little")
        )
    ...
    def join_match(self):
        ...
        play_deck_id = None
        if not resp == None and "Payload" in resp:
            for event_deck in json.loads(
                json.loads(resp["Payload"])["Preferences"]["RecentGamesData"]
            ):
        ...
        resp = self.inquire(400, {"DeckId": play_deck_id})  # getDeck (body)
        play_deck_body = None
        if not resp == None and "Payload" in resp:
...
class BattlefieldChatter(FrontdoorChatter):
    def __init__(
        self, host, port, certfile, client_version, match_id, fabric_uri, timeout=10.0
    ):
        self.request_id = 0
    ...
    def propose(self, ty: pb.ClientToMatchServiceMessageType, payload) -> str:
        ...
        trans_id = str(uuid.uuid1())
        self.speak(
            pb.ClientToMatchServiceMessage(
                requestId=self.request_id,
                timestamp=int(
                    (datetime.utcnow() - datetime(1, 1, 1)).total_seconds() * 10**7
                ),
    ...
    def get_gre_client_messages(self, trans_id=None):
        ...
        return [
            msg
            for trans in self.reply[trans_id]
            if hasattr(trans, "greToClientEvent")
